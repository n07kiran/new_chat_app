const socket = io()
console.log("chatRoom.scripst.js")
...
inputMessageForm.addEventListener("submit",(e)=>{
    ...
    if(input.value){
        ...
        if(checkForAiMessage(input.value)){
            // socket.emit("askAI",getAllMessages()) // giving context to whole conversation, do it later
...
function addUserJoined(user) {
    ...
    div.classList.add("center-justified-text-join");
    div.textContent = `${user.username} joined the chat!`;

    // const messages = document.getElementById("messages"); // Ensure messages is defined
    messages.appendChild(div);

    messages.scrollTo(0, messages.scrollHeight); // Correct scrolling behavior
}
...
function addUserLeft(user){
    const div = document.createElement("div");
    div.classList.add("center-justified-text-left");
